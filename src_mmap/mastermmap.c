...
int watchdog()
{
  ...
  char *log_files[2] = {"log/processAmmap.log", "log/processBmmap.log"};
...
int main() {
  ...
  // Open the log file
  if ((log_fd = open("log/mastermmap.log",O_WRONLY|O_APPEND|O_CREAT, 0666)) == -1)
  {
  ...
  // Process A
  char * arg_list_A[] = { "/usr/bin/konsole", "-e", "./bin/processAmmap", NULL };
  ...
  // Process B
  char * arg_list_B[] = { "/usr/bin/konsole", "-e", "./bin/processBmmap", NULL };
  ...
  int fd_pa = open("log/processAmmap.log", O_CREAT | O_RDWR, 0666);
  if (fd_pa == -1)
  {
    close(log_fd);
    perror("Error opening log file");
  ...
  int fd_pb = open("log/processBmmap.log", O_CREAT | O_RDWR, 0666);
  if (fd_pb == -1)
  {
    close(log_fd);
    close(fd_pa);
