...
int main(int argc, char **argv) {
  ...
  bmpfile_t *bmp; //active shared memory that the two process will acesses
  // Data type for defining pixel colors (BGRA)
  rgb_pixel_t pixel = {255, 0, 0, 0}; //fully blue allows us to see the circle 
  ...
  */
  ...
  int depth = 4;
  //creating the bitmap 
  //this will be shared memory between the two processes
  bmp = bmp_create(width, height, depth);//depth is 4 because we are using a colored image
